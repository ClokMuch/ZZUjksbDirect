...
def give_me_a_captcha_result(image_byte, sub_images_with_desc="./captcha_single/"):
    ...
    captcha_original = Image.open(image_byte)
    ...
    captcha_original_4 = captcha_original.crop((22 * 3, 0, 22 * 4, 24)).histogram()
    ...
    for each_original in captcha_original_crop_pool:
        for key, value in captcha_desc.items():
            tmp_image = Image.open(sub_images_with_desc + value[0]).histogram()
            un_comp = []
